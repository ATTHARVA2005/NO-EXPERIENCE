...
export async function POST(request: Request) {
  try {
    ...
    if (sessionId) {
      ...
      // FALLBACK: If Redis fails, load from Supabase
      if (!redisContext) {
    ...
    // Get recent messages from Redis or Supabase (last 10 for context window)
    const recentRedisMessages = redisContext?.messages.slice(-10) || []
    const recentSupabaseMessages = supabaseMessages.slice(-10) || []
    
    // Build conversation history (Redis takes priority, then Supabase, then provided history)
...
Tutor: Provide a helpful, concise explanation followed by ONE short check-for-understanding question.`
    ...
    console.error("[tutor-chat-enhanced] Error:", error)
